...
import { notifyAdminActivity } from '../lib/activityNotifications.ts';
...
export const Cart: React.FC = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const productIdsStr = cart.map(item => item.id).join(',');
      const orderResponse = await fetch('https://ymgyekgmonqhehmnskcw.supabase.co/functions/v1/create-razorpay-order', {
        ...
        body: JSON.stringify({ amount: Math.round(total * 100), productIds: productIdsStr })
      });
      ...
      const options = {
        ...
        handler: async (response: any) => {
          ...
          void notifyAdminActivity({
            title: 'User Purchase',
            message: `${user?.name || 'User'} completed checkout for ${totalItems} item(s), total ₹${total.toFixed(2)}.`,
          });
          ...
          <div className="w-full lg:w-[35%] sticky top-24">
              ...
              <div className="p-8 space-y-5">
                ...
                {user && cart.length > 0 && (
                  <button 
                    ...
                    disabled={verifying}
                    className="w-full h-12 bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold text-[11px] uppercase rounded-xl border border-blue-200 transition-colors flex items-center justify-center gap-2 mt-4 shadow-sm"
                  >
                    {verifying ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-sync-alt"></i>}
                    {verifying ? 'Verifying Payment...' : 'Verify Offline Payment'}
                  </button>
                )}
            ...
            <div className="mt-8 px-8 text-center">
              <p className="text-[11px] text-gray-400 font-bold uppercase leading-relaxed">
                By clicking confirm, you agree to DIGi QuRY's <span className="text-blue-500 cursor-pointer hover:underline">Digital Asset License</span> and <span className="text-blue-500 cursor-pointer hover:underline">Refund Policy</span>.
              </p>
            </div>
          </div>
        </div>
