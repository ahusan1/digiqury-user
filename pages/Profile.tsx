...
export const Profile: React.FC = () => {
  ...
  const fetchDeletionStatus = async () => {
    ...
    const { data } = await supabase
      ...
      .eq('user_id', user.id)
      .maybeSingle();
    if (data) setDeletionStatus(data.status);
  };
  ...
  const handleLogout = async () => {
    const confirmLogout = window.confirm("Are you sure you want to log out of your DIGi QuRY account?");
    if (confirmLogout) {
      await logout();
      navigate('/');
    }
  ...
  const handleDownloadData = async () => {
    try {
      ...
      a.href = url;
      a.download = `digiqury_data_${user.id}.json`;
      a.click();
  ...
  const handleRequestDeletion = async () => {
    ...
    const confirmDelete = window.confirm("Are you sure you want to permanently delete your account and all associated data? This action cannot be undone once approved.");
    ...
    try {
      ...
      toast.success("Account deletion request submitted for Admin review.");
    } catch (err: any) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="px-6 py-5 bg-blue-50/30 rounded-3xl border border-blue-50 font-black text-[#2874f0] text-sm flex items-center gap-3">
                      ...
                      Joined DIGi QuRY on {new Date(user.created_at).toLocaleDateString(undefined, { day: 'numeric', month: 'long', year: 'numeric' })}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex flex-col p-6 bg-emerald-50/50 rounded-[2rem] border border-emerald-100">
                      ...
                      <p className="text-[11px] text-emerald-700/70 font-bold leading-relaxed">
                        Your account is currently protected by DIGi QuRY's real-time digital asset monitoring.
                      </p>
                      <button 
                        onClick={handleDownloadData}
                        className="mt-6 text-[9px] font-black text-emerald-600 uppercase tracking-widest hover:underline text-left bg-white px-4 py-2 rounded-xl w-fit flex items-center gap-2"
                      >
                    ...
                    <div className="flex flex-col p-6 bg-red-50/50 rounded-[2rem] border border-red-100">
                      ...
                      {deletionStatus === 'pending' ? (
                        <span className="mt-6 text-[9px] font-black text-orange-600 uppercase tracking-widest bg-orange-100 px-3 py-2 rounded-xl w-fit flex items-center gap-2">
                          <i className="fas fa-clock"></i> Deletion Pending Admin Approval
                        </span>
                      ) : (
                        <button 
                          onClick={handleRequestDeletion}
                          className="mt-6 text-[9px] font-black text-red-600 uppercase tracking-widest hover:underline text-left bg-white px-4 py-2 rounded-xl w-fit"
                        >
